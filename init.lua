xtraores_redo  = {}
xtraores_redo.path = minetest.get_modpath("xtraores_redo")

dofile(xtraores_redo.path .. "/ores.lua")
dofile(xtraores_redo.path .. "/items.lua")
dofile(xtraores_redo.path .. "/tools.lua")
dofile(xtraores_redo.path .. "/armor.lua")
dofile(xtraores_redo.path .. "/oreblocks.lua")
dofile(xtraores_redo.path .. "/special_weapons.lua")
dofile(xtraores_redo.path .. "/other_blocks.lua")
dofile(xtraores_redo.path .. "/awards.lua")
...
local xo_proj_c_down = w_item:get_definition().projectile_cooldown or 20
...
local needed_ammo = w_item:get_definition().needed_ammo or ""
		...
		if not inv:contains_item("main", needed_ammo) then
			minetest.sound_play("xtraores_redo_empty", {object=player})
		end
...
if  inv:contains_item("main", needed_ammo) then
		...
		if pos and dir and yaw then
			pos.y = pos.y + 1.6
local proj_itself = w_item:get_definition().projectile_attack or "xtraores_redo:gunsmoke"
...
local p_spr = w_item:get_definition().projectile_spread / 10  or 10

obj:setvelocity({x=dir.x * p_vel, y=dir.y * p_vel, z=dir.z * p_vel})
obj:setacceleration({x=dir.x * math.random(0,p_spr), y=math.random(0,p_spr) - p_grav, z=dir.z * math.random(0,p_spr)})
